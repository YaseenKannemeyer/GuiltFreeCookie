...
const props = defineProps({
  ...
  id: { type: String, default: () => `field-${Math.random().toString(36).slice(2, 8)}` },
...
const fieldClass = computed(() => (props.variant === 'dark' ? 'field-dark' : 'field'))
...
<template>
  <div class="w-full">
    ...
    <textarea
      ...
      :class="[fieldClass, 'resize-none', error ? 'ring-2 ring-red-400 border-red-300' : '']"
      @input="onInput"
      @blur="$emit('blur', $event)"
      @focus="$emit('focus', $event)"
    />
    ...
    <!-- Error / Help text -->
    <p v-if="error" class="mt-2 text-sm text-red-500 flex items-center gap-1.5 animate-fade-in">
      <ExclamationCircleIcon class="h-4 w-4 shrink-0" />
      {{ error }}
    </p>
    <p
      v-else-if="help"
      :class="['mt-2 text-sm', variant === 'dark' ? 'text-cream/50' : 'text-chocolate/50']"
    >
      {{ help }}
    </p>
  </div>
